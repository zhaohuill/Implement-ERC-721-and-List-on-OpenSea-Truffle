...
contract("Date", (accounts) => {
    ...
    describe("Deployed Date", async () => {
        ...
        it("gifts the owner the origin of time", async () => {
            ...
            let meta = await date.readTokenMetaData(0)
        ...
        it("gifts the owner my birthday", async () => {
            ...
            let title = await date.tokenDateToTitle(1986, 6, 12)
            expect(title).to.equal("CBD")

            title = await date.tokenIdToTitle((1986-1)*372 + (6-1)*31 + 12-1)
            expect(title).to.equal("CBD")
        })
        ...
        it("gifts the owner elon's birthday", async () => {
            ...
            let title = await date.tokenDateToTitle(1971, 6, 28)
    ...
    describe("Minting a date", async () => {
        ...
        it("creates a token", async () => {
            reciept = await date.createNFT(1912, 6, 23, "Birthday Alan Turing", { from: accounts[1], value: price })
        ...
        it("sets the note", async () => {
            let note = await date.tokenDateToTitle(1912, 6, 23)
        ...
        it("prevents it from being minted again", async() => {
            await expectRevert(
                date.createNFT(1912, 6, 23, "Met my dog", { from: accounts[2], value: price }),
    ...
    describe("Trying to mint a date without paying", async () => {
        it("fails", async () => {
            await expectRevert(
                date.createNFT(2000, 1, 1, "The millenium!", { from: accounts[2], value: (price - price) }),
                "Creating a Date N.F.T. costs 10 finney"
            )
        })
    })
    ...
    describe("Trying to mint a future date", async () => {
        it("fails for one day in the future", async () => {
            ...
            await expectRevert(
                date.createNFT(tomorrow.getFullYear(), tomorrow.getMonth()+1, tomorrow.getDate(), "Tomorrow never dies!", { from: accounts[2], value: price }),
        ...
        it("fails for one month in the future", async () => {
            ...
            await expectRevert(
                date.createNFT(today.getFullYear(), today.getMonth()+2, today.getDate(), "Flaky time test!", { from: accounts[2], value: price }),
                "a date from the future can't be claimed"
            )
        })
        it("fails for one year in the future", async () => {
            ...
            let today = new Date((pendingBlock.timestamp) * 1000)
            await expectRevert(
                date.createNFT(today.getFullYear()+1, today.getMonth()+1, today.getDate(), "In a years time …", { from: accounts[2], value: price }),
                "a date from the future can't be claimed"
            )
        })
    })
    ...
    describe("Changing the title of a date", async () => {
        it("works if you are the owner of this date", async () => {
            await date.changeTitleOfTokenById(0, "This is the beginning")
            let title = await date.tokenIdToTitle(0)
            expect(title).to.equal("This is the beginning")

            await date.changeTitleOfTokenByDate(1986, 6, 12, "HBD2ME")
            title = await date.tokenDateToTitle(1986, 6, 12)
            expect(title).to.equal("HBD2ME")
        })

        it("reverts if you aren't the owner of this date", async () => {
            await expectRevert(
                date.methods["changeTitleOfTokenById(uint256,string)"](0, "this will fail", { from: accounts[1] }),
            ...
            await expectRevert(
                date.changeTitleOfTokenByDate(1986, 6, 12, "this will fail", { from: accounts[1] }),
                "only the owner of this date can change its title"
            )
            title = await date.tokenDateToTitle(1986, 6, 12)
            expect(title).to.equal("HBD2ME")
