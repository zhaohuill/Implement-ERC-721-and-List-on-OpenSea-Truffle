...
export function claimDate(date, note) {
    return contract.methods.createNFT(
        date.getFullYear(),
        date.getMonth() + 1,
        date.getDate(),
        note,
...
export async function loadToken(tokenId) {
    const tokenData = await contract.methods.readTokenMetaData(tokenId).call()
    const owner = await contract.methods.ownerOf(tokenId).call()

    //+-Return All the Information of the MetaData Struct of an N.F.T. in Json Format to show it in the User Interface:_
    return {
        tokenId: tokenId,
        date: (() => { 
          let d = new Date()
