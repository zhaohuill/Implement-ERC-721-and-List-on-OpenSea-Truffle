...
//+-This is an N.F.T. that is Minted using a Date, Title and Color.
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC721/ERC721.sol";

contract Date is Ownable, ERC721 {

    //+-Metadata that will be used to Identify the N.F.T. as unique.
    struct Metadata {
        uint16 year;
        uint8 month;
        uint8 day;
    ...
    function /**_*/baseURI() public view virtual override returns (string memory) {
        return _currentBaseURI;
    }

    //+-The Mint Function Takes a Year, Month, Day, a Color and a Title to create a new N.F.T..+-It is "Internal" and Normal Users can't call it.
    ...
    /**+-The CreateNFT Function uses the Mint Function Taking all the Data that the Users give to it except fot the Color that is choosen randomly to create a new N.F.T..
    +-It is "External" so Users can call it and "Payable" because users have to Pay a fee to create a new N.F.T..*/
    function createNFT(uint16 year, uint8 month, uint8 day, string calldata title) external payable {
        ...
        uint8 color;
        uint256 randomNum = pseudoRNG(year, month, day, title) % 1000000;
        if (randomNum < 1000) {
            color = 7;
        } else if (randomNum < 6000) {
            color = 6;
        } else if (randomNum < 16000) {
            color = 5;
        } else if (randomNum < 66000) {
            color = 4;
        } else if (randomNum < 166000) {
            color = 3;
        } else if (randomNum < 366000) {
            color = 2;
        } else if (randomNum < 666000) {
            color = 1;
        } else {
            color = 0;
        }
    ...
    function readTokenMetaData(uint256 tokenId) external view returns (uint16 year, uint8 month, uint8 day, uint8 color, string memory title) {
    ...
    function tokenIdToTitle(uint256 tokenId) external view returns (string memory) {
        require(_exists(tokenId), "token not minted");
        Metadata memory date = id_to_date[tokenId];
        return date.title;
    }

    function tokenDateToTitle(uint16 year, uint8 month, uint8 day) external view returns (string memory) {
        require(_exists(id(year, month, day)), "token not minted");
        Metadata memory date = id_to_date[id(year, month, day)];
        return date.title;
    }

    function changeTitleOfTokenByDate(uint16 year, uint8 month, uint8 day, string memory title) external {
        require(_exists(id(year, month, day)), "token not minted");
        changeTitleOfTokenById(id(year, month, day), title);
    }

    function changeTitleOfTokenById(uint256 tokenId, string memory title) public {
        require(_exists(tokenId), "token not minted");
